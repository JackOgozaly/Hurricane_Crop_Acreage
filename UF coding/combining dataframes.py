...
hurricane_df = pd.read_csv(
    r'https://raw.githubusercontent.com/JackOgozaly/Hurricane_Crop_Acreage/main/Data/historical_hurricane_date.csv')

# Use our function for the different wind speeds
hurricane_irma_ts = hurricane_shapefile(
    hurricane_df, 'AL112017', wind_speed=34)
hurricane_irma_tc = hurricane_shapefile(
    hurricane_df, 'AL112017', wind_speed=50)
...
county_shapes_12 = county_shapes[county_shapes['STATEFP'] == '12'].reset_index(
    drop=True)
...
county_shapes_12 = county_shapes_12.drop(
    columns=["COUNTYNS", "AFFGEOID", "GEOID", "LSAD", "ALAND", "AWATER"])
...
impacted_counties = gpd.overlay(
    pgdf, county_shapes_12,  how='intersection', keep_geom_type=True)
impacted_counties.plot()
impacted_counties["Imp_area"] = impacted_counties.geometry.area
impacted_counties['OverlapIntersection'] = (
    impacted_counties["Imp_area"] / impacted_counties["full_area"])*100
impacted_counties['Wind_Speed'] = '34kt Winds'
impacted_counties = impacted_counties.reindex(
    columns=['STATEFP', 'COUNTYFP', 'NAME', 'geometry', 'full_area', 'Imp_area', 'OverlapIntersection', 'Wind_Speed'])

# need to add 0 to N/A county before calculate
impacted_counties2 = gpd.overlay(
    county_shapes_12, p2gdf, how='intersection', keep_geom_type=True)
impacted_counties2["Imp_area"] = impacted_counties2.geometry.area
impacted_counties2.plot()

impacted_counties2_merged = pd.merge(county_shapes_12, impacted_counties2[[
                                     "COUNTYFP", "Imp_area"]], on='COUNTYFP', how='left')
impacted_counties2_merged["Imp_area"] = impacted_counties2_merged["Imp_area"].fillna(
    0)

impacted_counties2_merged['OverlapIntersection'] = (
    impacted_counties2_merged["Imp_area"] / impacted_counties2_merged["full_area"])*100
impacted_counties2_merged['Wind_Speed'] = '50kt Winds'

# need to add 0 to N/A county before calculate
impacted_counties3 = gpd.overlay(
    county_shapes_12, p3gdf, how='intersection', keep_geom_type=True)
impacted_counties3["Imp_area"] = impacted_counties3.geometry.area
impacted_counties3.plot()

impacted_counties3_merged = pd.merge(county_shapes_12, impacted_counties3[[
                                     "COUNTYFP", "Imp_area"]], on='COUNTYFP', how='left')
impacted_counties3_merged["Imp_area"] = impacted_counties3_merged["Imp_area"].fillna(
    0)

impacted_counties3_merged['OverlapIntersection'] = (
    impacted_counties3_merged["Imp_area"] / impacted_counties3_merged["full_area"])*100
impacted_counties3_merged['Wind_Speed'] = '64kt Winds'

# Appending three dfs
impacted_counties_final = impacted_counties.append(
    [impacted_counties2_merged, impacted_counties3_merged])

# Wind to Columns
df_wind_col = impacted_counties_final.groupby(['COUNTYFP', 'Wind_Speed'])[
    ['Imp_area']].sum().reset_index()
df_wind_col = df_wind_col.set_index(
    ['COUNTYFP', 'Wind_Speed']).Imp_area.unstack(fill_value='')

# Merged datasets
df_merged_final = pd.merge(
    county_shapes_12, df_wind_col, how='left', on='COUNTYFP')
df_merged_final['34kt_pct'] = (
    df_merged_final["34kt Winds"] / df_merged_final["full_area"])*100
df_merged_final['50kt_pct'] = (
    df_merged_final["50kt Winds"] / df_merged_final["full_area"])*100
df_merged_final['64kt_pct'] = (
    df_merged_final["64kt Winds"] / df_merged_final["full_area"])*100
df_merged_final = df_merged_final.reindex(columns=['STATEFP', 'COUNTYFP', 'NAME', 'geometry',
                                          'full_area', '34kt Winds', '34kt_pct', '50kt Winds', '50kt_pct', '64kt Winds', '64kt_pct'])
df_merged_final = df_merged_final.drop(columns=["geometry"])


# Cause of loss
cause_of_loss = pd.read_parquet(
    'https://github.com/JackOgozaly/Hurricane_Crop_Acreage/blob/main/Data/Cause_Of_Loss/crop_loss_data_4.parquet.gzip?raw=true')
cause_of_loss = cause_of_loss.rename(
    columns={'county_code': 'COUNTYFP', 'state_code': 'STATEFP'})

# Cause of loss clean
cause_of_loss_clean = cause_of_loss[cause_of_loss['year_of_loss'] == 2017]
cause_of_loss_clean = cause_of_loss_clean[(cause_of_loss_clean['month_of_loss'] == '8') | (
    cause_of_loss_clean['month_of_loss'] == '9')]
cause_of_loss_clean = cause_of_loss_clean[cause_of_loss_clean['cause_of_loss_description']
                                          == 'Hurricane/Tropical Depression']
cause_of_loss_clean = cause_of_loss_clean[cause_of_loss_clean['STATEFP'] == '12'].reset_index(
    drop=True)
cause_of_loss_clean1 = cause_of_loss_clean.drop(cause_of_loss_clean.columns[[
                                                0, 3, 5, 6, 7, 8, 9, 13, 14, 16, 19, 20, 21, 22, 23, 26, 27]], axis=1)

cause_of_loss_clean_group = cause_of_loss_clean1.groupby(['COUNTYFP'])[
    ['total_premium', 'indemnity_amount', 'net_planted_quantity', 'liability', 'net_determined_quantity']].sum()

# Merge df_merged_final with cause of loss
final_data = pd.merge(df_merged_final, cause_of_loss_clean_group,
                      on='COUNTYFP', how='left').fillna(0)
...
data1 = final_data_pre.drop(columns=["NAME", "STATEFP", "COUNTYFP", "total_premium",
                            "net_planted_quantity", "net_determined_quantity", "liability"])
# rearrange the columns as indemnity_amount first column
data1 = data1.iloc[:, [7] + list(range(7)) +
                   list(range(8, len(data1.columns)))]


data1 = data1.astype("double")
data1 = data1.rename(columns={"64kt_pct": "sixfour_pct",
                     "34kt_pct": "threefour_pct", "50kt_pct": "fifity_pct"})
